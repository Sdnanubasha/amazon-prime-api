...
let mystuffModel = require('../models/movies-model');
let db;
...
const createMyStuff = async (array, res) => {
    try {
      console.log(array);
      let { m_id = 0,title = 'Example',image = '',description = '',ratings = 5,releaseYear = 2018} = array[0];
...
const MystuffMovies = async (req,res)=>{
    try{
      let mystuff_id = Number(req.query.id);
      db.collection('movies').find({m_id: mystuff_id ? mystuff_id : "nE"})
