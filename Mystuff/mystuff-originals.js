...
let mystuffModel = require('../models/original-model');
let db;
...
const createMyStuff = async (array, res) => {
    try {
      console.log(array);
      let { a_o_id = 0,title = 'Example',image = '',description = '',ratings = 5,releaseYear = 2018} = array[0];
...
const MystuffOriginals = async (req,res)=>{
    try{
      let mystuff_id = Number(req.query.id);
      db.collection('amazon_originals').find({a_o_id: mystuff_id ? mystuff_id : "nE"})
