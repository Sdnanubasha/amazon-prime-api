...
let mystuffModel = require('../models/kids-model');
let db;
...
const createMyStuff = async (array, res) => {
    try {
      console.log(array);
      let { k_id = 0,title = 'Example',image = '',description = '',ratings = 5,releaseYear = 2018} = array[0];
...
const MystuffKids = async (req,res)=>{
    try{
      let mystuff_id = Number(req.query.id);
      db.collection('kids').find({k_id: mystuff_id ? mystuff_id : "nE"})
