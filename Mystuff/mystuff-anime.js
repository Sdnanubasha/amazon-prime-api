...
let mystuffModel = require('../models/anime-model');
let db;
...
const createMyStuff = async (array, res) => {
    try {
      console.log(array);
      let { a_id = 0,title = 'Example',image = '',description = '',ratings = 5,releaseYear = 2018} = array[0];
...
const MystuffAnimes = async (req,res)=>{
    try{
      let mystuff_id = Number(req.query.id);
      db.collection('anime').find({a_id: mystuff_id ? mystuff_id : "nE"})
